...
async def cmd_plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(ai.plans_intro(), reply_markup=join_keyboard())
...
async def cmd_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            ...
            text=content,
            reply_markup=join_keyboard("Join VIP - Soma Full Version")
        )
...
async def cb_confirm_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not plan_key or not plan:
        await query.edit_message_text("Session imeexpire. Anza tena na /start")
        ...
        try:
            await context.bot.send_message(
                ...
                text=f"New subscriber!\n"
                     f"Name: {user.first_name}\n"
                     f"Plan: {plan['label']} - KSH {plan['price']:,}\n"
                     f"Phone: {phone}\n"
                     f"Expires: {expiry.strftime('%d %B %Y')}"
            )
        except Exception:
...
async def cb_random_story(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("Naandika story...")
    story = ai.generate_story()
    await context.bot.send_message(
...
async def post_init(application: Application):
    ...
    job_queue.run_repeating(job_promo, interval=10800, first=5400)
