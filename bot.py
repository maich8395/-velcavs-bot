...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"👤 {user.first_name}\n"
        f"📋 Plan: {plan_cfg.get('emoji','📋')} {plan_cfg.get('label', sub['plan'])}\n"
        f"📅 Expires: {expiry.strftime('%d %B %Y')}\n"
        f"⏳ Days left: {days_left}{warning}",
        parse_mode="Markdown",
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    s = db.get_stats()
    breakdown = "\n".join(
        f"  {config.PLANS[k]['emoji']} {config.PLANS[k]['label']}: {v}"
        for k, v in s["plan_breakdown"].items()
    ) or "  Hakuna bado"
    await update.message.reply_text(
        f"📊 *Velcavs Stats*\n\n"
        f"👥 Total: {s['total']}\n"
...
async def cmd_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✅ Story imepost!")
    except Exception as e:
        await update.message.reply_text(f"❌ Imefail: {e}")
...
async def cb_plan_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan_key not in config.PLANS:
        await query.answer("Plan haikupatikana. Jaribu tena.", show_alert=True)
        return CHOOSING_PLAN
    ...
    await query.edit_message_text(
        ...
        f"_(Mfano: 07XXXXXXXX au 01XXXXXXXX)_",
        parse_mode="Markdown",
    )
...
async def handle_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    instructions = ai.payment_instructions(
        user.first_name, f"{plan['emoji']} {plan['label']}",
        plan["price"], phone, config.MPESA_PAYBILL,
    )
...
async def cb_confirm_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.upsert_subscriber(
        ...
        first_name=user.first_name, phone=phone,
        plan=plan_key, amount=plan["price"], expiry=expiry,
    )
...
async def cb_random_story(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=story,
        reply_markup=story_request_keyboard(),
    )
    ...
    for sub in db.get_expiring_soon(hours=24):
        ...
        try:
            await bot.send_message(
                ...
                parse_mode="Markdown", reply_markup=renew_keyboard(),
            )
            db.mark_expiry_notified(sub["user_id"])
        except Exception as exc:
            logger.warning(f"Could not warn {sub['user_id']}: {exc}")
